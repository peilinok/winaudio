...
- **Wire SetClientProperties / ducking into Application Loopback in the same change** — originally rejected. Reopened 2026-08-21: Application Loopback now calls SetClientProperties when Stream params `clientProperties.enabled` is true, using the same category / offload / stream-option mapping as the Wasapi scaffold (including IsOffloadCapable when offload is on). Failure fails open. Ducking stays unwired (render-only). Stream init still does not own client properties.

## Consequences

- Stream init sits after endpoint or Application Loopback activation and before ducking / Start. It does not own the pump. Application Loopback applies client properties after activation and before Stream init.
