...
第一阶段实现了 **WASAPI-Shared 采集/播放**；waveIn/waveOut 与 WASAPI-Exclusive 作为后续阶段。
...
- 能力范围（已实现）：
  - **WASAPI-Shared**：`IAudioClient` 采集与播放，共享模式（多应用可并行使用设备）。
...
- 能力范围（后续阶段）：
  ...
  - 格式转换/重采样（当前要求输入 WAV 格式与设备混音格式一致）。
...
# 运行单元测试（gtest）
.\x64\Debug\WinAudioTests.exe                    # 全部测试
