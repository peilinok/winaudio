...
powershell -ExecutionPolicy Bypass -File $buildWrapper -PrintEnvironmentSummary `
  cmake -S $root -B $BuildDir -DWINAUDIO_ENABLE_HARDWARE_VALIDATION_TESTS=ON
...
Write-Host "`n=== CTest ==="
ctest --test-dir $BuildDir -C $Config -R hardware_validation_test --output-on-failure
